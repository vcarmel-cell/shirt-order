...
2. **בחירת מוטו** - בחירה מרשימה קבועה, או הצעה חופשית ("אחר - אני מציע/ה מוטו משלי")
3. **פרטי הזמנה** - שם, מידה, צבע/עיצוב

כל התשובות (כולל המוטו) נשמרות באותו מסמך באוסף `shirt-orders`.

## עריכת אפשרויות הטופס

בתוך `index.html`, באובייקט `EVENT` בתחילת ה-`<script>`:
- `mottoOptions` - רשימת המוטואים לבחירה (כרגע placeholder - **יש להחליף ברשימה הסופית**)
